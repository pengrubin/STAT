...
covaryear <- data.frame(scaled_year = ydat[, 1])
covar2 <- data.frame(scaled_year = ydat[, 1], SOI = ydat[, 2])
evd_fit1 <- fgev(fremantle[,2], nsloc = covaryear)
evd_fit2 <- fgev(fremantle[,2], nsloc = covar2)
